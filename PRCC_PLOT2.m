...
barh(r)
set(gca,'YLim',[.5 5.5]);% This automatically sets the
% XLimMode to manual.

% Set XTick so that only the integer values that
% range from 0.5 - 12.5 are used.
set(gca,'YTick',[1:5]); % This automatically sets
...
%PRCC_var={'alpha','T','c_tra_prop','T0', 'eta','b', 'a'};%
% ylabel=['\alpha    ';'T         ';'c_{traced}';'T_0       ';'\eta      ';'\theta    ';'a         '];
%ylabel=['$\alpha    $';'$T         $';'$c_{traced}$';'$T_0$       ';'$\eta$      '];
ylabel=['Testing adherence rate   ';'Days per round of testing';'Tracing percentage       ';...
    'Free transmission days   ';'Testing efficiency       '];
