...
beta_LHS=LHS_Call(0.06, 0.08, 0.12, 0 ,runs,'unif'); %1
c1_LHS=LHS_Call(10, 13.8, 20, 0 ,runs,'unif'); %2
c2_LHS=LHS_Call(5,  10.5, 15, 0 ,runs,'unif'); %3
% alpha_LHS=abs(LHS_Call(0.7,0.95, 1, 0 ,runs,'unif')); %4
% c_alpha_LHS=abs(LHS_Call(0.6, 0.95, 1, 0,runs,'unif')); %5

% T_stage1_LHS=3:1:12;
% T_stage1_LHS=repmat(T_stage1_LHS,1,runs/10);
par1guess=[beta_LHS c1_LHS c2_LHS];
...
% 
% parfor i=1:runs
%      [par1,fval] = fmincon(@lse_error_sol_FJ,par1guess(i,:),[],[],[],[],lb,ub,[],options,FJ_data,par2);
%      PAR1(i,:)=par1;
% end
...
[T1,X1] =ode45(@Zero_s2_Fujian,[1:0.1:length(FJ_case)],x1,[],par1,par2,options_ode2);

% x2=X1_1(end,:);
% par2_2=par2;
% par2_2(7)=1;
% [T1,X1_2] =ode45(@Zero_s2,[T1(end):0.1:length(FJ_case)],x2,[],par1,par2_2,options_ode2);
% X1=[X1_1;X1_2(2:end,:)];
% 
daily_new_model=[FJ_daily_case(1);diff(X1(1:10:end,14))];
cum_case_model=X1(1:10:end,14);
isolated_model=X1(1:10:end,13);
% size(daily_new_model)
% size(FJ_daily_case)
daily_case_sim(i,:)=daily_new_model';
m = norm(daily_new_model-FJ_daily_case)^2;
2
% SSR(i)=sum((daily_new_model-FJ_daily_case).^2)/(sum((FJ_daily_case-mean(FJ_daily_case)).^2));
SSR(i)=sum((cum_case_model-FJ_case).^2)/(sum((FJ_case-mean(FJ_case)).^2))+sum((isolated_model-FJ_isolated).^2)/(sum((FJ_isolated-mean(FJ_isolated)).^2));
...
figure(1);clf
plot(1:1:length(FJ_daily_case),yy(1,:),'linewidth',1.5)
hold on
plot(1:1:length(FJ_daily_case),yy(2,:),'linewidth',1.5)
hold on
plot(1:1:length(FJ_daily_case),yy(3,:),'linewidth',1.5)
hold on
plot(1:1:length(FJ_daily_case),FJ_daily_case,'ob')
xticks([1 13 25])
xticklabels({'Mar 3','Mar 16','Mar 28'})
...
eval(['name_figure1 = ','''','Fitting_Fujian'''])
...
% best fit
par1guess(id(1),:)
 save par1_FJ_mean_1 'par1_mean'
 save par1_FJ_var_1 'par1_var'
