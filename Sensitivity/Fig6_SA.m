...
FJ_case=FJ_data(1:25,4);
FJ_daily_case=FJ_data(1:25,3);
...
FJ_daily_q=FJ_data(1:16,5);
  ...
  T0_LHS=LHS_Call(5,10,14, 0 ,runs,'unif');%4
   ...
   alpha_2_LHS=LHS_Call(0.2,1,1, 0 ,runs,'unif'); %6
%  LHSmatrix=[alpha_LHS T_LHS c_tra_prop_LHS  T0_LHS eta_LHS b_LHS a_LHS];
  LHSmatrix=[alpha_LHS T_LHS c_tra_prop_LHS T0_LHS  eta_LHS alpha_2_LHS];
...
%least
par1=[0.0646345731806166	15.3191825197524	11.0578753017759	0.00996619785378382];
...
%     par2=[4559797 LHSmatrix(x,6)  2      2    LHSmatrix(x,7)   1/6  LHSmatrix(x,2)  21    LHSmatrix(x,5)   LHSmatrix(x,1)     7       13     2.34 1.32  7   15      0.278   ];
     par2=[4559797    0.75          2      2    0.8              1/6  LHSmatrix(x,2)  21    LHSmatrix(x,5)   LHSmatrix(x,1)     7       13     2.34 1.32  7   15      0.278   1    LHSmatrix(x,6)                ];
...
options_ode = odeset('NonNegative',1:6,'RelTol',1e-6,'AbsTol',1e-6,'Refine',6);
...
l=0;
tn1=1:0.1:T;
options_ode2 = odeset('NonNegative',1:16,'RelTol',1e-6,'AbsTol',1e-6);
...
y_var={'Cumulative Confirmed Cases'};
...
y_var={'Cumulative Isolated people'};
